...
class InvoiceController extends Controller
{
    public function store(Request $request){
        ...
        $order_id = 'order-' . '78523691456' . $request->user()->id . $cart_ids->implode('');
        ...
        if ($invoiceExists) {
            ...
            $data = [
                ...
                'item_details' => $request->collect('carts')->map(fn ($item) => [
                    "id" => $item['id'],
                    // "price" => round((11/100) * $item['price'], 0) + $item['price'],
                    "price" => $item['price'],
                    "quantity" => 1,
            ...
            $response = Http::withBasicAuth(config('services.midtrans.server_key') . ':', '')->post('https://api.sandbox.midtrans.com/v2/charge', $data);
            $body = $response->json();
            $invoice->update([
                'payment_info' => [
